...
QuestionBankModel.prototype.chooseQuestion = function() {
	...
	var n1=base.randNumb1;
	var n2=base.randNumb2;
	var n3=base.randNumb3;
	var n4=base.randNumb4;
	
	var color=base.randName1;
	...
	this.question ="class Point ~ ^  attr_accessor :x, :y ~ ^  def initialize(x,y) ~^^^ @x = x ~^^^ @y = y ~^end ~^def addUp ~^^^@x + @y ~^end ~end ~~class ColorPoint < Point~^attr_accessor :color~^def initialize(x,y,c='clear') ~^^^ super(x,y) ~^^^@color = c ~^ end ~^def addUp ~^^^ 2 * @x + 2 * @y ~^end~end ~~ p = Point.New("+n1+", "+n2+") ~ cp = ColorPoint.New("+n3+", "+n4+", '"+color+"')";
	var bb=base.randomQuestion;
	$("#qqq1").html(bb).css("font-weight", "bold"); 
	$("#qqq").html( "<p>What is the value of ans after the Ruby code above is evaluated?</p>" ); 
